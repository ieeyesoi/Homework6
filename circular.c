...
int isEmpty(QueueType *cQ)
{
      if (cQ->front == cQ->rear) { // 큐가 비어있을 때 (front, rear의 값이 같음 = 큐에 요소가 존재하지 X)
         printf("원형 큐가 비어있습니다");   //해당 문구 출력함
            return 1; 
      } else return 0; // 같지 않다면 0을 리턴함
}

//큐가 가득 채워져있는지 확인하는 함수
int isFull(QueueType *cQ)
{   
   if ((cQ->rear + 1) % MAX_QUEUE_SIZE == cQ->front) //큐가 가득 채워져있을 때
    { 
        printf("원형 큐가 가득 채워져있습니다");
      return 1; 
   } else return 0; // 같지 않다면 0을 리턴함
}
...
void enQueue(QueueType *cQ, element item)
{
   if (isFull(cQ)) { // 큐가 가득 찼을 때 
      ...
      cQ->rear = (cQ->rear + 1) % MAX_QUEUE_SIZE;   // rear를 다음으로 이동시킴, 크기 초과하면 배열의 처음으로 인덱스 변경함
      
      cQ->queue[cQ->rear] = item; // 새로운 요소를 현재 rear 위치에 저장함
   }
}
...
{   
   ...
   cQ->front = (cQ->front + 1) % MAX_QUEUE_SIZE; // front 다음 인덱스를 현재의 front로 할당함
   // 실제로는 요소를 큐에서 제거하지 않지만 공백 상태로 봄(실제로는 큐가 남아있음)
    return; 
   }
}
...
void printQ(QueueType *cQ)
{
   ...
   i = first;   // 반복문에 사용할 변수 i를 first의 값으로 초기화함
   while(i != last){ // 변수 i가 last 값에 도달할 떄까지 반복함
      printf("%3c", cQ->queue[i]); // %3c으로 간격을 두며 큐의 맨 처음 값을 출력함
